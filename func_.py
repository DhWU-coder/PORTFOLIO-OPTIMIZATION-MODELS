...
def encoder(inputs, n_features, ff_dim=32, num_heads=2, dropout=0):
    x = inputs
...
def build_transformer_model3(seq_length, n_features, block_num=1, num_heads=2, ff_dim=32,
                             dropout=0, encoder_dropout=0.2, LSTM_dim=32):
    inputs = Input(shape=(seq_length, n_features))
    ...
    # encoder
    for _ in range(block_num):
        x = encoder(x, 16, ff_dim, num_heads, dropout)
...
def evaluate_portfolio_(portfolio_df):
    ...
    # annual_return_ = np.mean(1 + portfolio_df['return']) ** 365
...
def evaluate_portfolio(portfolio_df, benchmark_df):
    if len(portfolio_df) <= 1:
        return {'annual_return': 0, 'sharpe_ratio': 0, 'max_drawdown': 0, 'win_rate': 0, 'excess_return': 0,
                'information_ratio': 0}

    portfolio_df['cumulative_return'] = (1 + portfolio_df['return']).cumprod()

    # Align benchmark_df with portfolio_df dates
    benchmark_df = benchmark_df[benchmark_df['Date'].isin(portfolio_df['date'])].copy()
    if len(benchmark_df) == 0:
        return {'annual_return': 0, 'sharpe_ratio': 0, 'max_drawdown': 0, 'win_rate': 0, 'excess_return': 0,
                'information_ratio': 0}

    benchmark_df['cumulative_return'] = (1 + benchmark_df['daily_return'].fillna(0)).cumprod()

    days = (portfolio_df['date'].max() - portfolio_df['date'].min()).days
    if days <= 0:
        annual_return = 0
    else:
        annual_return = (portfolio_df['cumulative_return'].iloc[-1] ** (365 / days)) - 1
    ...
    win_rate = (portfolio_df['return'] > 0).mean() if not portfolio_df['return'].empty else 0

    portfolio_returns = portfolio_df['return'].values
    benchmark_returns = benchmark_df['daily_return'].values
    ...
    if min_length == 0:
        ...
        information_ratio = np.mean(excess_returns) / np.std(excess_returns) * np.sqrt(252) if np.std(
...
def plot_cumulative_returns_(lstm_portfolio, transformer_portfolio, lstm_transformer_portfolio, benchmark_df, save_dir='pic'):
    plt.figure(figsize=(10, 6))

    if len(lstm_portfolio) > 0:
        plt.plot(lstm_portfolio['date'], lstm_portfolio['cumulative_return'],
                 color='blue', linestyle='-', label='LSTM Portfolio')
    if len(transformer_portfolio) > 0:
        plt.plot(transformer_portfolio['date'], transformer_portfolio['cumulative_return'],
                 color='red', linestyle='-',
                 label='Transformer Portfolio')
    if len(lstm_transformer_portfolio) > 0:
        plt.plot(lstm_transformer_portfolio['date'], lstm_transformer_portfolio['cumulative_return'],
                 color='orange', linestyle='-',
                 label='LSTM+Transformer Portfolio')

    plt.axhline(y=1.0, color='black', linestyle='--', label='1.0 Benchmark')
    # plt.axhline(y=1.0, color='g', linestyle='--', label='Benchmark')
    # Ensure benchmark_df covers the full date range of the portfolios
    full_date_range = pd.date_range(start='2022-02-15',
                                    end=max(lstm_portfolio['date'].max(), transformer_portfolio['date'].max()),
                                    freq='D')
    benchmark_df = benchmark_df.set_index('Date').reindex(full_date_range, method='ffill').reset_index()
    benchmark_df = benchmark_df.rename(columns={'index': 'Date'})
    benchmark_df['daily_return'] = benchmark_df['daily_return'].fillna(0)
    benchmark_df['cumulative_return'] = (1 + benchmark_df['daily_return']).cumprod()
    plt.plot(benchmark_df['Date'], benchmark_df['cumulative_return'], 'g--', label='CSI 300 Benchmark')
...
def plot_drawdowns_(lstm_portfolio, transformer_portfolio, lstm_transformer_portfolio, save_dir='pic'):
    plt.figure(figsize=(10, 6))

    if len(lstm_portfolio) > 0:
        plt.fill_between(lstm_portfolio['date'], 0, lstm_portfolio['drawdown'],
                         color='blue', alpha=0.3,
                         label='LSTM Drawdown')
    if len(transformer_portfolio) > 0:
        plt.fill_between(transformer_portfolio['date'], 0, transformer_portfolio['drawdown'],
                         color='red', alpha=0.3,
                         label='Transformer Drawdown')
    if len(transformer_portfolio) > 0:
        plt.fill_between(lstm_transformer_portfolio['date'], 0, lstm_transformer_portfolio['drawdown'],
                         color='orange', alpha=0.3,
                         label='LSTM+Transformer Drawdown')
...
def plot_return_distribution_(lstm_portfolio, transformer_portfolio, lstm_transformer_portfolio, save_dir='pic'):
    plt.figure(figsize=(10, 6))
    ...
    if len(transformer_portfolio) > 0:
        plt.hist(transformer_portfolio['return'], bins=20, alpha=0.5, label='Transformer Returns', color='red')
    if len(transformer_portfolio) > 0:
        plt.hist(lstm_transformer_portfolio['return'], bins=20, alpha=0.5, label='LSTM+Transformer Returns',
                 color='orange')
